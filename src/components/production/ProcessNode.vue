...
defineProps({ process: { type: Object, required: true }, index: { type: Number, required: true }, last: Boolean })
defineEmits(['select'])
</script>

<template>
  <div class="process-step" :class="[`is-${process.status}`, { 'is-last': last }]">
    <button class="process-node" type="button" @click="$emit('select', process)">
      ...
      <span class="node-content">
        <span class="node-title"><strong>{{ process.name }}</strong><el-tag :type="statusMeta[process.status].type" size="small" effect="dark">{{ statusMeta[process.status].label }}</el-tag></span>
