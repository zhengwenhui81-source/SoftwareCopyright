...
defineEmits(['select'])
const baseDisplayStatus = computed(() => props.process.baseStatus || props.process.status || 'waiting')
const riskDisplayStatus = computed(() => props.process.runtimeAlarmState || (props.process.manualException ? 'current_abnormal' : null))
const cardDisplayStatus = computed(() => riskDisplayStatus.value || baseDisplayStatus.value)
const baseVisualMeta = computed(() => statusMeta[baseDisplayStatus.value] || statusMeta.waiting)
const riskVisualMeta = computed(() => ({
  current_abnormal: { label: baseDisplayStatus.value === 'completed' ? '参数异常' : '当前异常', type: 'danger' },
  recovery_pending: { label: '待恢复确认', type: 'warning' },
})[riskDisplayStatus.value] || null)
const primaryVisualMeta = computed(() => riskVisualMeta.value || baseVisualMeta.value)
const baseStateNote = computed(() => riskDisplayStatus.value && baseDisplayStatus.value === 'completed' ? '工序已完成' : '')
</script>

<template>
  <div class="process-step" :class="[`is-${cardDisplayStatus}`, `base-${baseDisplayStatus}`, { 'is-last': last }]">
    <button class="process-node" type="button" @click="$emit('select', process)">
      ...
      <span class="node-content">
        <span class="node-title"><strong>{{ process.name }}</strong><el-tag :type="primaryVisualMeta.type" size="small" effect="dark">{{ primaryVisualMeta.label }}</el-tag></span>
        <span v-if="baseStateNote" class="base-state-note">{{ baseStateNote }}</span>
      ...
      <span v-if="process.alarmDetail" class="alarm-parameter"><em>{{ riskDisplayStatus === 'recovery_pending' ? '参数已恢复 · 待确认' : `${baseDisplayStatus === 'completed' ? '完成后参数异常' : '当前异常'}：${process.alarmDetail.parameter}` }}</em><b>触发值 {{ process.alarmDetail.value }}</b><b v-if="process.alarmDetail.currentValue">当前值 {{ process.alarmDetail.currentValue }}</b><small>仿真范围 {{ process.alarmDetail.threshold }}</small></span>
      <span v-if="baseDisplayStatus === 'running'" class="progress"><i :style="{ width: `${process.progress}%` }"></i></span>
...
.is-current_abnormal .process-node{border:1px solid #e85454;box-shadow:inset 0 0 25px rgba(232,84,84,.12)}.is-current_abnormal .node-icon{color:#ff6767;border-color:#b74646;background:rgba(232,84,84,.12)}.is-recovery_pending .process-node{border:1px solid #e6a23c;box-shadow:inset 0 0 25px rgba(230,162,60,.12)}.is-recovery_pending .node-icon{color:#ffbd5a;border-color:#a97932;background:rgba(230,162,60,.12)}.alarm-parameter small{color:#b88989;font-size:8px}.is-recovery_pending .alarm-parameter{color:#ffc66f;background:rgba(230,162,60,.1);border-color:#e6a23c}.is-recovery_pending .alarm-parameter b{color:#ffe0aa}.is-recovery_pending .alarm-parameter small{color:#b89661}
.base-state-note{align-self:flex-end;margin-top:-3px;color:#91aabd;font-size:9px}.is-current_abnormal .base-state-note{color:#ffaaaa}.is-recovery_pending .base-state-note{color:#ffd18a}
</style>
