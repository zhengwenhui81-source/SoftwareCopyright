...
const statusMap = { pending: { label: '待确认', type: 'warning' }, processing: { label: '处理中', type: 'primary' }, closed: { label: '已关闭', type: 'info' } }
...
<template>
  <section class="event-panel">
    ...
    <el-table :data="events" empty-text="当前批次暂无生产异常事件">
      ...
      <el-table-column label="操作" width="205" fixed="right"><template #default="{ row }"><el-button text type="primary" size="small" @click="showDetail(row)">查看详情</el-button><el-button v-if="row.status === 'pending'" text type="warning" size="small" @click="$emit('confirm', row.id)">确认事件</el-button><el-button v-if="row.status === 'processing'" text type="danger" size="small" @click="$emit('close', row.id)">关闭事件</el-button><span v-if="row.status === 'closed'" class="closed">处理完成</span></template></el-table-column>
    </el-table>

    <el-dialog v-model="detailVisible" width="620px" title="生产异常事件详情">
      <template v-if="selectedEvent"><div class="detail-head"><div><small>{{ selectedEvent.id }} · {{ selectedEvent.batchId }}</small><h3>{{ selectedEvent.title }}</h3></div><el-tag :type="statusMap[selectedEvent.status]?.type">{{ statusMap[selectedEvent.status]?.label }}</el-tag></div><el-descriptions :column="2" border><el-descriptions-item label="钢种">{{ selectedEvent.steelGrade }}</el-descriptions-item><el-descriptions-item label="异常工序">{{ selectedEvent.process }}</el-descriptions-item><el-descriptions-item label="异常参数">{{ selectedEvent.parameter }}</el-descriptions-item><el-descriptions-item label="当前值">{{ selectedEvent.currentValue }} {{ selectedEvent.unit }}</el-descriptions-item><el-descriptions-item label="标准范围" :span="2">{{ selectedEvent.threshold }}</el-descriptions-item><el-descriptions-item label="异常说明" :span="2">{{ selectedEvent.description }}</el-descriptions-item></el-descriptions><div class="suggestion"><h4>处理建议</h4><ul><li v-for="item in selectedEvent.suggestion" :key="item">{{ item }}</li></ul></div></template>
